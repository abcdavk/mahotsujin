import { EntityComponentTypes, EquipmentSlot, ItemStack, system, TicksPerSecond, world, } from "@minecraft/server";
const magicWeapons = [
    {
        id: "dave:wand",
...
system.afterEvents.scriptEventReceive.subscribe(({ sourceEntity, message, id }) => {
    ...
    if (inv && con && itemHand && id === 'dave:add_magic') {
        if (!itemHand.hasTag("dave:spell_book"))
            return;
        let spellBook = spellBooks.find((item) => item.id === itemHand.typeId);
        if (!spellBook)
            return;
        let spellAtributes = spellsRegistry[parseInt(message)];
        let spellOnItem = itemHand.getDynamicProperty("dave:spell_binding");
        if (spellOnItem) {
            let spellArray = JSON.parse(spellOnItem);
            if (spellArray.length + 1 > spellBook.max_spells)
                return;
            spellArray.push(parseInt(message));
            let itemLores = [`§r§7Spells: (${spellArray.length}/${spellBook.max_spells})`];
