import { Entity, Dimension, Vector3, system } from "@minecraft/server";
import { magicParticles } from "../constants/magicParticles";

export class CreateParticle {
  private dimension: Dimension;
  private entity: Entity;
  private spellSelected: number;
  private lastTickPercent: number = 0;

  constructor(entity: Entity, dimension: Dimension, spellSelected: number) {
    this.entity = entity;
    this.dimension = dimension;
    this.spellSelected = spellSelected;
  }

  private getForwardVector(distance: number): Vector3 {
  ...
  spawnByTick(currentTick: number, completeTick: number) {
    if (this.spellSelected === undefined) return;
    ...
    particles.forEach((particle, index, arr) => {
      ...
      const shouldSpawn =
        (tickPercent >= start && this.lastTickPercent < start) || 
